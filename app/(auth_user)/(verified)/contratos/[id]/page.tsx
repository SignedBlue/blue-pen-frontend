...
export default async function SingleContract({ params }: { params: { id: string } }) {
  const contract_data = await fetch(`${backendUrl}/contracts/${params.id}`, {
    cache: "no-cache"
  });
  ...
  return <SingleContractSection contract={contract} user={user} />;
}